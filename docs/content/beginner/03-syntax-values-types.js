  ...
  window.RUST_COURSE_CHAPTERS.beginner.push({
          ...
          sections: [
            lesson({
              ...
              cppComparison: [
                ["C++ 的 `int`、`long` 宽度依平台而定，Rust 把位宽写进类型名，跨平台行为一致。`usize` 对应 `size_t`；`char` 在 C++ 是一个字节，在 Rust 是 4 字节的 Unicode scalar，要表示一个原始字节用 `u8`。但这并不意味着 Rust 字符串更占空间：`String`/`&str` 底层是 UTF-8 字节序列，ASCII 字符仍然只占 1 字节，和 C++ 的 `std::string` 一样；只有按 `char` 存储（`Vec<char>`、`[char; N]`）时才会每个字符固定占 4 字节。", "C++ 整数溢出是未定义行为，Rust 把它定义清楚了：debug build 直接 panic，release build 按二进制补码 wrap，并提供 `checked_/wrapping_/saturating_/overflowing_` 系列方法显式选择行为。"],
                ["C++ `int`/`long` widths are platform-dependent; Rust bakes the width into the type name for consistent cross-platform behavior. `usize` maps to `size_t`; `char` is one byte in C++ but a 4-byte Unicode scalar in Rust, so use `u8` for a raw byte. This does not make Rust strings larger: `String`/`&str` are UTF-8 byte sequences where ASCII still takes one byte per character, just like C++ `std::string`; only `char` storage such as `Vec<char>` or `[char; N]` uses a fixed four bytes per character.", "Integer overflow is UB in C++ but defined in Rust: debug builds panic, release builds wrap with two's complement, and the `checked_/wrapping_/saturating_/overflowing_` methods let you choose explicitly."]
              ],
              examples: [
                withMistakes(
