  ...
  window.RUST_COURSE_CHAPTERS.beginner.push({
          ...
          sections: [
            lesson({
              ...
              syntax: [
                ["整数把位宽和符号写进类型名：`i8/i16/i32/i64/i128/isize` 有符号，`u8/u16/u32/u64/u128/usize` 无符号；不标类型时默认推断成 `i32`。`usize`/`isize` 按指针宽度走，专门用于下标、长度和容量。", "浮点只有 `f32` 和 `f64`（默认 `f64`）；`bool` 占 1 字节；`char` 占 4 字节，存的是一个 Unicode scalar value：也就是一个范围在 U+0000 到 U+10FFFF、且不落在代理区（U+D800 到 U+DFFF）的 Unicode 码点，所以一个 `char` 能放下一个字母、一个汉字或一个 emoji。字面量可以用 `_` 分隔（`1_000_000`）、写进制（`0xff`、`0o77`、`0b1010`）、带后缀（`42u8`、`3.14f32`）。", "复合类型里，tuple 把若干不同类型的值打包成一个值，`()` 是空 tuple（unit 类型）；array 是 `[T; N]`，定长、同类型、连续存放，越界访问会 panic 而不是读到脏数据。"],
                ["Integers bake width and sign into the type name: `i8/i16/i32/i64/i128/isize` are signed, the `u*` family is unsigned, and an unannotated literal defaults to `i32`. `usize`/`isize` are pointer-sized and used for indices, lengths, and capacities.", "Floats are only `f32`/`f64` (default `f64`); `bool` is one byte; `char` is four bytes and stores a Unicode scalar value: a Unicode code point from U+0000 to U+10FFFF that is not a surrogate (U+D800 to U+DFFF), so one `char` holds a single letter, CJK character, or emoji. Literals allow `_` separators, `0x/0o/0b` bases, and type suffixes such as `42u8` or `3.14f32`.", "For compound types, a tuple packs several differently typed values into one value (`()` is the unit type), and an array `[T; N]` is fixed-length, same-typed, and contiguous; out-of-bounds access panics instead of reading garbage."]
              ],
              engineering: [
              ...
              cppComparison: [
                ["C++ 的 `int`、`long` 宽度依平台而定，Rust 把位宽写进类型名，跨平台行为一致。`usize` 对应 `size_t`；`char` 在 C++ 是一个字节，在 Rust 是 4 字节的 Unicode scalar，要表示一个原始字节用 `u8`。", "`char` 的 4 字节不会让字符串变大：Rust 的字符串 `String`/`&str` 是 UTF-8 编码的字节序列，不是 `char` 数组——ASCII（英文）字符仍然每个 1 字节，和 C++ 的 `std::string` 占用一样；只有显式按 `char` 存储（`Vec<char>`、`[char; N]`）时，每个字符才固定占 4 字节，这种用法很少见。", "C++ 整数溢出是未定义行为，Rust 把它定义清楚了：debug build 直接 panic，release build 按二进制补码 wrap，并提供 `checked_/wrapping_/saturating_/overflowing_` 系列方法显式选择行为。"],
                ["C++ `int`/`long` widths are platform-dependent; Rust bakes the width into the type name for consistent cross-platform behavior. `usize` maps to `size_t`; `char` is one byte in C++ but a 4-byte Unicode scalar in Rust, so use `u8` for a raw byte.", "A 4-byte `char` does not make strings larger: Rust's `String`/`&str` are UTF-8 byte sequences, not arrays of `char` — ASCII characters still take one byte each, just like C++ `std::string`; only explicit `char` storage such as `Vec<char>` or `[char; N]` uses a fixed four bytes per character, which is rare.", "Integer overflow is UB in C++ but defined in Rust: debug builds panic, release builds wrap with two's complement, and the `checked_/wrapping_/saturating_/overflowing_` methods let you choose explicitly."]
              ],
              examples: [
                withMistakes(
